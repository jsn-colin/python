...
SourcePath = input('源文件目录：')
DestPath = input('保存到目录：')

UC_PATH = SourcePath  # 缓存路径 例 D:/CloudMusic/Cache/
MP3_PATH = DestPath   # 存放歌曲路径
    ...
    def do_transform(self):
        ...
        for file in files:
            if file[-3:] == '.uc':  # 后缀.uc结尾为歌曲缓存
                ...
                song_id = self.get_songid_by_filename(file)   # 获取歌曲的id，就是缓存文件第一列的数字
